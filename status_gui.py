...
# anchor the status bar to the east (right side)
...
def forward(image_number):
    ...
    # add functionality to the status bar
    # pass the image_number variable into the status var from the function parameter 
    status = Label(root, text="Image " + str(image_number) + " of " + str(len(img_lst)), bd=1, relief=SUNKEN, anchor=E)
...
# add pady to the button_forward var for space between the status bar
button_back.grid(row=1,column=0)
button_exit.grid(row=1,column=1)
button_forward.grid(row=1,column=2,pady=10)

# place the status bar on the grid
# use sticky to stretch from west to east
status.grid(row=2,column=0,columnspan=3,sticky=W+E)
root.mainloop()